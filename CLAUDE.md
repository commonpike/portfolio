...
An exporter that answers to HTTP as well keeps the difference in four places and nowhere else, each behind `cli()`: `parameters()` reads `getopt()` or `$_GET` (unknown and `CLI_ONLY` query keys are dropped), `fail()` writes usage to stderr and exits 1 or replies `400` with a JSON error body, `headers()` — `Content-Type` plus `Access-Control-Allow-Origin: ALLOW_ORIGIN`, so a front end on another origin may read it — is called only when not on the CLI, and `listing()` lifts the execution limit to `SCAN_SECONDS` before a cold scan. The error replies send those headers too, or a browser would hide the error body. Everything after that is SAPI-agnostic. A value that arrives repeated or as an array (`--rank=1 --rank=2`, `?rank[]=1`) fails the same way a malformed one does, rather than being silently coerced.

That limit is raised in the exporter rather than in `Portfolio::projects()`, and only for the run that actually reads the asset root: a scan of an archive on a network mount outlives a web server's `max_execution_time`, while the CLI has no limit to lift — `set_time_limit()` there would only cap an otherwise unbounded export. The library stays out of it, as it stays out of every other environment concern.
